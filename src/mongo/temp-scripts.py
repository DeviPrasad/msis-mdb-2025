...
db.loan_purpose_param.drop()
db.accounts.drop()



db.loan_purpose_master.drop()
db.createCollection("loan_purpose_master")
db.loan_purpose_master.insertMany([
...
db.loan_purpose_master.find()


db.loan_purpose_param.drop()
db.createCollection("loan_purpose_param", {
  validator: {
    $jsonSchema: {
...
db.loan_purpose_param.insertMany([
    {
...
db.loan_purpose_param.estimatedDocumentCount()
db.loan_purpose_param.countDocuments()

function getLoanPurposeParamForRate(rate) {
    let params = [];

    const cursor = db.loan_purpose_param.find();
...
function getLoanPurposeParamForRate(rate) {
    ...
    const cursor = db.loan_purpose_param.find({ rateOfInterest: { $gte: rate } });
    while (cursor.hasNext()) {
        const p = cursor.next();
        params.push({
            id: p.purposeId,
...
function getLoanPurposeParamForRateSorted(purposeId, rate) {
    let params = [];
...
getLoanPurposeParamForRateSorted("farming", 7)
